...
# prepare the graph data objects for the Cords dataset
...
def generate_data(region_index=0, degree_limit=20, graph_type='extended', prepare_folder="graph_objects_degree_20_prepare"):
    ...
    df_seeds = pd.read_csv(data_dir+"/random_seeds_1534.txt", sep=" ", header=None)
    cur_random_seed = df_seeds[0].tolist()[region_index]

    random.seed(cur_random_seed)

    data_dicts  = data_features("cords_d20", "both", graph_type)
    ...
    # no need to call this line below if it is the first run
    # it needs to be called explicitly if the processed folder already contains data objects
    # dataset.process()
