...
# prepare the graph data objects for the mpfc dataset three images
...
parser.add_argument('--degree_limit', type=int, default=380, help='desired average degree of nodes')
...
def generate_data(region_index=0, degree_limit=20, graph_type='extended'):
    ...
    df_seeds = pd.read_csv(data_dir+"/random_seeds_3.txt", sep=" ", header=None)
    cur_random_seed = df_seeds[0].tolist()[region_index]

    random.seed(cur_random_seed)

    data_dicts  = data_features("mpfc_d20", "all", graph_type)
